...
var bagHu = new Bag('hu', 'standard', dictionaryHu);
...
describe('Solver', function () {
  ...
  describe('hu', function () {
    ...
    it('should find solutions if the line is blocked in front of the word', function () {
      var board = new Board();
      var score = new Score(board, dictionaryHu);
      var rack = new Rack();
      var solver = new Solver(board, score, dictionaryHu);
      ...
      var solutions = solver.solve(rack);
      solutions = solutions.filter(function (s) { return s.x === 5 && s.y === 6;});
      // console.log(solutions);
      // console.log(board.printBoard().join('\n'));
      // console.log(x, w, dzs);
      assert.equal(solutions.length, 2);
      assert.equal(solutions[0].word, 'hX');
      assert.equal(solutions[1].word, 'hE');
    });
  });
