...
Bag.prototype.initialize = function () {
  ...
  var types = language.get('type');

  if (types.has(this.bagType) === false) {
  ...
  var letterDistribution = types.get(this.bagType);
  var i;

  this.origianalTiles = [];

  this.blankSelection = language.get('extraBlankValues').toArray();

  for (i = 0; i < letterDistribution.size; i += 1) {
    var thisDistribution = letterDistribution.get(i);
    var j;
    var letter = thisDistribution.get('letter');
    if (this.blankSelection.indexOf(letter) === -1) {
      this.blankSelection.push(letter);
    }
  ...
  this.blankSelection.sort(function (a, b) { return a.localeCompare(b); });
