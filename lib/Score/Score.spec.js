...
var fs = require('fs');
var enDict = JSON.parse(fs.readFileSync(__dirname + '/../Dictionary/en/WL2014.json'));
...
describe('Score', function() {
  ...
  it('should get score for word using SQUARE method', function () {
    var board = new Board();
    var score = new Score(board, dictionary, 'SQUARE');
    // move
    var move = [
  ...
  it('should get score for single letter cross word using SQUARE method', function () {
    var board = new Board();
    var score = new Score(board, dictionary, 'SQUARE');
    // move
    var move = [
      {x: 6, y: 8, tile: new Tile('n', 1)},
    ];
    // set tiles
    board.setTile(5, 7, new Tile('b', 3));
    board.setTile(6, 7, new Tile('e', 1));
    board.setTile(7, 7, new Tile('g', 2));
    var result = score.getScore(move);
    assert.equal(result.success, true);
    assert.equal(result.score, 4);
    assert.equal(result.word, 'en');
  });
  ...
  describe('x direction', function () {
    ...
    it('should get score 54 for 8-letter word (seatings)', function () {
      ...
      assert.equal(result.isBingo, false);
    });
    ...
    it('should get score +50 points for 7-letter word (seating)', function () {
      ...
      assert.equal(result.isBingo, true);
    });
