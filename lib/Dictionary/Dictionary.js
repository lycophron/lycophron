...
function Dictionary(dict, id) {
  ...
  if (typeof dict === 'string') {
    // TODO: how would this work from the browser? use superagent
    // TODO: if extension is not defined will this work?
    this.dict = require(__dirname + '/' + dict);
  } else {
    this.dict = dict;
  }
...
Dictionary.prototype.getSubWords = function (subword, exactMatch, node, letters, numBlanks, usedBlanks) {
    ...
    if (!node) {
        ...
        // strip blanks
        subword = subword.LFilter(function (i) {
    ...
    if (usedBlanks < numBlanks) {
        // use one joker
        nodeLetters = Object.keys(node).LSort();
        // ASSUMPTION: nodelLetters are sorted alphabetically
        for (i = 0; i < nodeLetters.length; i += 1) {
            key = nodeLetters[i];
            if (key !== '_') {
                results = results.LConcat(this.getSubWords(subword, exactMatch, node[key], letters + key, numBlanks, usedBlanks + 1));
                if (node[key]._ !== undefined) {
                    results = results.LConcat(node[key]._);
                }
    ...
    if (letter) {
        ...
        if (node[letter] !== undefined) {
            results = results.LConcat(this.getSubWords(nextSubword, exactMatch, node[letter], letters + letter, numBlanks, usedBlanks));
            if (node[letter]._ !== undefined) {
                results = results.LConcat(node[letter]._);
            }
        }

        if (node._ !== undefined) {
            results = results.LConcat(node._);
        }
...
Dictionary.prototype.getSolutionForProblem = function (letters, patterns) {
    ...
    if (patterns) {
      results.solution = results.solution.LFilter(function (value) {
        var result = true;
        if (patterns.length) {
