...
function init(app, logger, config) {
    ...
    logger.info('Google OAuth2 is initializing');
    if (config.authentication.GOOGLE_CLIENT_ID && config.authentication.GOOGLE_CLIENT_SECRET) {
        ...
        if (config.authentication.allowGuests) {
            logger.error('Google OAuth2 is failed to initialize. ' +
            'GOOGLE_CLIENT_ID and GOOGLE_CLIENT_SECRET have to be defined config.authentication.GOOGLE_*');
            useGoogle = false;
        } else {
            throw new Error('Enable guests config.authentication.allowGuests=true or ' +
            'GOOGLE_CLIENT_ID and GOOGLE_CLIENT_SECRET have to be defined config.authentication.GOOGLE_*');
        }
    }
    ...
    if (useGoogle) {
        ...
        logger.info('Google OAuth2 is ready');
    }
